...
class DLGNSF(torch.nn.Module):
    def __init__(self, args):
        ...
        self.fc1f = nn.ModuleList([nn.Linear(in_features=self.args.input_size, out_features=self.args.channels[1], bias=True) for _ in range(self.args.k)])
        self.fc1v = nn.Linear(in_features=self.args.channels[0], out_features=self.args.channels[1], bias=True)
        
        self.fc2f = nn.ModuleList([nn.Linear(in_features=self.args.input_size, out_features=self.args.channels[2], bias=True) for _ in range(self.args.k)])
        self.fc2v = nn.Linear(in_features=self.args.channels[1], out_features=self.args.channels[2], bias=True)

        self.fc3f = nn.ModuleList([nn.Linear(in_features=self.args.input_size, out_features=self.args.channels[3], bias=True) for _ in range(self.args.k)])
        self.fc3v = nn.Linear(in_features=self.args.channels[2], out_features=self.args.channels[3], bias=True)

        self.fc4f = nn.ModuleList([nn.Linear(in_features=self.args.input_size, out_features=self.args.channels[4], bias=True) for _ in range(self.args.k)])
    ...
    def forward(self, x):
        ...
        k_preact = [fc1f(x) for i, fc1f in enumerate(self.fc1f)]
        ...
        k_preact = [fc2f(x) for i, fc2f in enumerate(self.fc2f)]
        ...
        k_preact = [fc3f(x) for i, fc3f in enumerate(self.fc3f)]
        ...
        k_preact = [fc4f(x) for i, fc4f in enumerate(self.fc4f)]
