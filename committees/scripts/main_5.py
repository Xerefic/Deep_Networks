...
num_committees = np.arange(30, 100, 10).tolist() + np.arange(100, 1100, 100).tolist()
...
def train_dnn(seeds, betas, lrs, weight_decays, logger):
    ...
    with Pool(min(os.cpu_count(), 20)) as p:
...
if __name__ == '__main__':
    ...
    handler = logging.FileHandler('log_5.csv', mode='w')
    ...
    if train_networks['dnn']:
        ...
        runs.to_csv('dnn_5.csv', index=False)
    ...
    if train_networks['dlgn']:
        ...
        runs = pd.DataFrame(runs)
        runs.to_csv('dlgn_5.csv', index=False)
    ...
    if train_networks['committee']:
        ...
        runs = pd.DataFrame(runs)
        runs.to_csv('committee_5.csv', index=False)
